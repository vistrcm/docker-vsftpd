...
Note: this is fork of https://github.com/fauria/docker-vsftpd
...
### Installation from [Docker registry hub](https://hub.docker.com/r/vistrcm/vsftpd/).

You can download the image with the following command:

```bash
docker pull vistrcm/vsftpd
```
...
```bash
  docker run --rm vistrcm/vsftpd
```

2) Create a container in active mode using the default user account, with a binded data directory:

```bash
docker run -d -p 21:21 -v /my/data/directory:/home/vsftpd --name vsftpd vistrcm/vsftpd
...
--name vsftpd --restart=always vistrcm/vsftpd
```
